...
But the contribution of that variable will be negiglibe if nothing is done.
...
summary(m)
```
It shows that `Area.I1` has a load under 0.6, which means its contribution to DEA model is negiglibe.
...
Note that the maximum value of a variable load is the maximun number of variables of its types, so `load.max = 4` has no effect on results.

Now load level raises to the given value of 0.6, efficiency average decreases a litte.
