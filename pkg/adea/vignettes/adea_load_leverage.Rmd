...
The follwing call tests all groups of two units
```{r}
...
Each call to `adea_load_leverage` requires to solve a big set of a large linear program, so is a very demanding of computation resource, and can requiere a very large time, so be patient.
